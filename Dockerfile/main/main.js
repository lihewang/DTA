...
var scen_Loop = function () {
    ...
    async.series([
        ...
        function (callback) {
            ...
            volRunFinished = fs.createWriteStream('/output/runfinished');
            var s = 'A, B, TIMESTEP';
            ...
            function (callback) { 
                ...
                if (iter < par.maxiter && maxGap > par.gap) { //continue
                    ...
                    async.series([
                        ...
                        function (callback) {
                            //upload to bucket
                            bucket.upload(loadedLinkFile, { destination: '/output/' + runListPar.runs[scenIndex] + '/vol.csv' }, function (err, result) {
                                if (err) {
